# frozen_string_literal: true

require_dependency "decidim/components/namer"

Decidim.register_component(:study_notes) do |component|
  component.engine = Decidim::StudyNotes::Engine
  component.admin_engine = Decidim::StudyNotes::AdminEngine
  component.icon = "decidim/study_notes/icon.svg"

  component.on(:before_destroy) do |instance|
    raise StandardError, "Can't remove this component" if Decidim::StudyNotes::StudyNote.where(component: instance).any?
    raise StandardError, "Can't remove this component" if Decidim::StudyNotes::Category.where(component: instance).any?
  end

  # These actions permissions can be configured in the admin panel
  # component.actions = %w()

  # component.settings(:global) do |settings|
  #   # Add your global settings
  #   # Available types: :integer, :boolean
  #   # settings.attribute :vote_limit, type: :integer, default: 0
  # end
  component.settings(:global) do |settings|
    settings.attribute :block_comments, type: :boolean
  end

  # component.settings(:step) do |settings|
  #   # Add your settings per step
  # end

  component.settings(:custom) do |settings|
    settings.attribute :locations, type: :text
  end

  # component.register_resource(:some_resource) do |resource|
  #   # Register a optional resource that can be references from other resources.
  #   resource.model_class_name = "Decidim::StudyNotes::SomeResource"
  #   resource.template = "decidim/study_notes/some_resources/linked_some_resources"
  # end

  # component.register_stat :some_stat do |context, start_at, end_at|
  #   # Register some stat number to the application
  # end

  # component.seeds do |participatory_space|
  #   # Add some seeds for this component
  # end
end
