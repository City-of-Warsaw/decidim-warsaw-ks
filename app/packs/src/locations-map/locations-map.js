...
var nominatimUrl = Decidim.config.get('nominatimUrl');
...
function addMarker(e, id = Date.now()) {
  ...
  $('#map + .locations').append(`<p style="max-width: 800px; margin-top: 10px; display: flex; justify-content: space-between; align-items: center;">
    <input type="text" readonly id="input-location-${id}" value="${addressText}" class="added-marker" style="width: 84%;" />
    <input type="button" id="location-delete-${id}" value="usuń pinezkę" style="background: var(--primary); padding: 5px; color: white; border-radius: 5px; cursor: pointer; height: 40px; width: 15%;" />
  </p>`);
  ...
  // $('#search').hide();
  $('#map').siblings('.auto-search-wrapper').hide();
}
...
new Autocomplete("search", {
  ...
  onSearch: ({ currentValue }) => {
    ...
    // const api = '../static/search.json'
      const api = `${nominatimUrl}/search?format=geojson&limit=5&addressdetails=1&countrycodes=pl&q=${encodeURI(
      currentValue
    )}`;
