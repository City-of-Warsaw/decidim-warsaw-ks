        ...
        .each(function () {
          ...
          $.extend(MultiFile, {
            ...
            trigger: function (event, element, MultiFile, files) {
              var rv,
                handler = MultiFile[event] || MultiFile["on" + event];
              if (handler) {
          ...
          MultiFile.addSlave = function (slave, slave_count) {
            ...
            slave.change(function (a, b, c) {
                    ...
                    $.each(FILE_LIST(this), function (i, file) {
                      if (file.name) {
                        ...
                        if (v == x || v == x.substr(x.length - v.length)) {
                          ...
                          MultiFile.trigger("FileDuplicate", e, MultiFile, [
                            file,
                          ]);
                        }
                      }
              ...
              if (
                ...
                ERROR[ERROR.length] = MultiFile.STRING.toomany.replace(
                  "$max",
                  MultiFile.max
                );
              ...
              if (MultiFile.maxsize > 0 && total_size > MultiFile.maxsize) {
                ERROR[ERROR.length] = MultiFile.STRING.toomuch.replace(
                  "$size",
                  sl(total_size) + " > " + sl(MultiFile.maxsize)
                );
          ...
          MultiFile.addToList = function (slave, slave_count, files) {
              ...
              var v = String(file.name || "").replace(/[&<>'"]/g, function (c) {
                ...
                label = $(
                  (
                    ...
                    n +
                    "</span>" +
                    (MultiFile.preview || $(slave).is(".with-preview")
                      ? p
                      : "") +
                    "</span>"
                  )
            ...
            MultiFile.STRING.remove = MultiFile.STRING.remove.replace(
              /\$(file|name)/gi,
              filesNames.join(", ")
            ); // mod. CS
            ...
            var r = $('<div class="MultiFile-label"></div>'),
              b = $(
                '<button type="button" class="MultiFile-remove flex-shrink-0 text-sm text-red-500 cursor-pointer remove-file-btn">' +
            ...
            // MOD CS: add alerting for adding a new file
            MultiFile.list.after(`<span class="show-for-sr" role="alert" id="alert-for-${filesNames.join("+")}">Załącznik został dodany</span>`);
            setTimeout(() => document.getElementById(`alert-for-${filesNames.join("+")}`).remove(), 20000);
	...
	*/
    $.extend($.fn.MultiFile, {
      ...
      reset: function () {
        ...
        if (mf) $(mf.list).find("button.MultiFile-remove").click();
        return $(this);
      },
