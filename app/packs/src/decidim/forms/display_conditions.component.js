...
class DisplayCondition {
  ...
  getInputValue() {
    const $conditionWrapperField = $(
      `.question[data-question-id='${this.conditionQuestion}']`
    );
    const $textInput = $conditionWrapperField.find(
      "textarea, input[type='text']:not([name$=\\[custom_body\\]])"
    );
    ...
    $conditionWrapperField
      .find(".js-radio-button-collection, .js-check-box-collection")
      .find(".js-collection-input")
      .each((idx, el) => {
...
class DisplayConditionsComponent {
  ...
  mustShow() {
    ...
    const mandatoryConditions = conditions.filter(
      (condition) => condition.mandatory
    );
    const nonMandatoryConditions = conditions.filter(
      (condition) => !condition.mandatory
    );
  ...
  showQuestion() {
    ...
    // If so, don't remove the disabled attribute to maintain form state
