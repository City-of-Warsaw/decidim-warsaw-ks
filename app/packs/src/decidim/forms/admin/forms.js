...
export default function createEditableForm() {
  ...
  const displayConditionFieldSelector =
    ".questionnaire-question-display-condition";
  const displayConditionsWrapperSelector =
    ".questionnaire-question-display-conditions";
  const displayConditionRemoveFieldButtonSelector = ".remove-display-condition";

  const displayConditionQuestionSelector =
    "select[name$=\\[decidim_condition_question_id\\]]";
  const displayConditionAnswerOptionSelector =
    "select[name$=\\[decidim_answer_option_id\\]]";
  const displayConditionTypeSelector = "select[name$=\\[condition_type\\]]";
  const deletedInputSelector = "input[name$=\\[deleted\\]]";

  const displayConditionValueWrapperSelector =
    ".questionnaire-question-display-condition-value";
  const displayconditionAnswerOptionWrapperSelector =
    ".questionnaire-question-display-condition-answer-option";
  ...
  const removeDisplayConditionsForFirstQuestion = () => {
    $(fieldSelector).each((idx, el) => {
      ...
      if (idx) {
        $question
          .find(displayConditionsWrapperSelector)
          .find(deletedInputSelector)
          .val("false");
  ...
  const autoLabelByPosition = new AutoLabelByPositionComponent({
    ...
    onPositionComputed: (el, idx) => {
      $(el)
        .find("input[name$=\\[position\\]]:not([name*=\\[matrix_rows\\]])")
        .val(idx);
  ...
  const createSortableList = () => {
    const $container = $(".questionnaire-questions-list:not(.published)");

    if ($container.length === 0) {
      return;
    }

    const existingHandlers = $container.find(".card-divider").data("events");

    $container.find(".card-divider").off("dragstart.sortable dragend.sortable");
    $container.off("dragenter.sortable dragover.sortable drop.sortable");
    $container.find(".drag-placeholder").remove();

    const $draggingElements = $container.find(
      ".questionnaire-question.dragging"
    );

    $container.find(".questionnaire-question").removeClass("dragging").css({
      opacity: "",
      "z-index": "",
    });
    ...
    const $handles = $container.find(".card-divider");

    $handles.each(function (index) {
      const $handle = $(this);
      const $item = $handle.closest(".questionnaire-question");

      $handle.attr("draggable", true);
      $handle.css("cursor", "move");

      $handle.on("dragstart.sortable", function (e) {
        sortableState.draggedElement = $item;
        $item.addClass("dragging");

        $container.find(".drag-placeholder").remove();

        sortableState.placeholder = $('<div class="drag-placeholder"></div>');
        $item.after(sortableState.placeholder);

        e.originalEvent.dataTransfer.effectAllowed = "move";
        e.originalEvent.dataTransfer.setData("text/html", $item.attr("id"));
      ...
      $handle.on("dragend.sortable", function (e) {
    ...
    $container.on("dragover.sortable", function (e) {
      e.preventDefault();
      e.stopPropagation();
      e.originalEvent.dataTransfer.dropEffect = "move";

      if (!sortableState.draggedElement || !sortableState.placeholder) {
        return;
      }
      ...
      $container
        ...
        .not(sortableState.placeholder)
        .remove();

      if ($afterElement === null) {
        if (!sortableState.placeholder.is($container.children().last())) {
          $container.append(sortableState.placeholder);
        }
      } else {
        if (!sortableState.placeholder.is($afterElement.prev())) {
          $afterElement.before(sortableState.placeholder);
        }
      }
    });

    $container.on("drop.sortable", function (e) {
      e.preventDefault();
      ...
      sortableState.placeholder.before(sortableState.draggedElement);
      sortableState.placeholder.remove();
      sortableState.placeholder = null;
      sortableState.draggedElement = null;
    ...
    function getDragAfterElement($container, y) {
      const $draggableElements = $container.find(
        ".questionnaire-question:not(.dragging)"
      );
  ...
  const createDynamicFieldsForAnswerOptions = (fieldId) => {
    ...
    const autoSelectOptions =
      createAutoMaxChoicesByNumberOfAnswerOptions(fieldId);
  ...
  const onDisplayConditionTypeChange = ($field) => {
    ...
    const $answerOptionWrapper = $field.find(
      displayconditionAnswerOptionWrapperSelector
    );
  ...
  const setupInitialQuestionAttributes = ($target) => {
    ...
    createFieldDependentInputs({
      ...
      enablingCondition: ($field) => {
        return (
          $field.val() === "multiple_option" ||
          $field.val() === "matrix_multiple"
        );
      },
    });
    ...
    dynamicFieldsForAnswerOptions[fieldId] =
      createDynamicFieldsForAnswerOptions(fieldId);
    dynamicFieldsForMatrixRows[fieldId] =
      createDynamicFieldsForMatrixRows(fieldId);
    dynamicFieldsForDisplayConditions[fieldId] =
      createDynamicFieldsForDisplayConditions(fieldId);
    ...
    const onQuestionTypeChange = () => {
      if (isMultipleChoiceOption($fieldQuestionTypeSelect.val())) {
        const nOptions = $fieldQuestionTypeSelect
          .parents(fieldSelector)
          .find(answerOptionFieldSelector).length;
      ...
      if (isMatrix($fieldQuestionTypeSelect.val())) {
        const nRows = $fieldQuestionTypeSelect
          .parents(fieldSelector)
          .find(matrixRowFieldSelector).length;
  ...
  createDynamicFields({
    ...
    duplicateQuestionButtonSelector: ".add-question-after", // custom
    ...
    TitleAndDescriptionTemplateSelector:
      ".decidim-title-and-description-template",
    ...
    onAddField: ($field) => {
      ...
      const shouldReinit =
        !sortableState.initialized || $field.find(".card-divider").length > 0;
      ...
      document.dispatchEvent(
        new CustomEvent("ajax:loaded", { detail: $field[0] })
      );
    },
    onRemoveField: ($field) => {
      autoLabelByPosition.run();
