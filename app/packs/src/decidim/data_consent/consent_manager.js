// overwritten file
// ignore iframe disabling when data-consent-ignore attribute is present
...
class ConsentManager {
  ...
  triggerIframes() {
    if (this.allAccepted() || this.externalAccepted()) {
      ...
      document.querySelectorAll("iframe").forEach((original) => {
        // ignore iframe disabling when data-consent-ignore attribute is present
        if (original.dataset.consentIgnore !== undefined) {
          return;
        }
  ...
  externalAccepted() {
    return this.categories.some((category) => {
      return category === "external" && this.state[category] === true;
    });
  }
