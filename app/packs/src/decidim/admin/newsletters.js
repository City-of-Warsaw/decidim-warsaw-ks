...
$(() => {
  ...
  if ($form.length > 0) {
    ...
    // custom - additional_followers
    const $sendNewsletterToTagsFollowers = $form.find("#newsletter_send_to_tags");
    const $sendNewsletterToNgoFollowers = $form.find("#newsletter_send_to_type_ngo");
    const $tagsForSelect = $form.find("#tags_for_select");
    ...
    $sendNewsletterToAllUsers.on("change", (event) => {
      ...
      if (checked) {
        ...
        // custom - additional_followers
        $sendNewsletterToTagsFollowers.find("input[type='checkbox']").prop("checked", !checked);
        $sendNewsletterToNgoFollowers.find("input[type='checkbox']").prop("checked", !checked);
        $tagsForSelect.hide();
      } else {
        $sendNewsletterToFollowers.find("input[type='checkbox']").prop("checked", !checked);
        $sendNewsletterToParticipants.find("input[type='checkbox']").prop("checked", !checked);
    ...
    $sendNewsletterToTagsFollowers.on("change", (event) => {
      const checked = event.target.checked;

      if (checked) {
        $sendNewsletterToFollowers.find("input[type='checkbox']").prop("checked", false);
    ...
    $sendNewsletterToNgoFollowers.on("change", (event) => {
      const checked = event.target.checked;
