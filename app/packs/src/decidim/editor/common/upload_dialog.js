...
export default class UploadDialog {
  constructor(element, { i18n, onOpen, onClose }) {
    ...
    this.values = { src: null, alt: null, description: null };
    ...
    this.dropZone.parentNode
      .querySelector(".upload-modal__text")
      .classList.add("mb-0");
    ...
    const toggleDragover = (active) => {
      if (active) {
        ...
        this.dropZone
          .querySelectorAll(".upload-modal__dropzone")
          .forEach((el) => el.classList.add("is-dragover"));
      } else {
    ...
    const handleClose = () => {
      ...
      const descriptionInput = this.inputSection.querySelector(".attachment-description");
      if (descriptionInput) {
        this.values.description = descriptionInput.value;
      }
    ...
    this.element.addEventListener("close.dialog", () =>
  ...
  toggle(values = {}, options = {}) {
    ...
    return new Promise((resolve) => {
      ...
      const titleInputHtml = `
        <form class="form-defaults form gap-6 flex flex-col">
          <label>
            ${options.inputLabel}
          ...
          <label>
            ...
            <input class="attachment-description" type="text" name="description">
          </label>
        </form>
      `;

      let titleSection = null;
      titleSection = createElement(
        `<div class="form__wrapper">${titleInputHtml}</div>`
      );
      titleSection.querySelector(".attachment-title").value = values.alt || "";
      titleSection.querySelector(".attachment-description").value =
        values.description || "";
