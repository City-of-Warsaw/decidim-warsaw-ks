// overwritten file
// remove rule to parse images only within [data-image] divs
  ...
  return new Promise((resolve, reject) => {
    fetch(uploadUrl, {
      ...
      .then((response) => {
        ...
        return new Promise((responseResolve) =>
          responseResolve({ message: i18n.uploadError })
        );
      })
      .then((json) => resolve({ title: fileNameToTitle(image.name), ...json }))
      .catch(reject);
  });
};
      ...
      new Plugin({
        props: {
          handlePaste(view, event) {
            ...
            Promise.all(
              images.map((item) =>
                uploadImage(item.getAsFile(), uploadImagesPath)
              )
            ).then((uploadedImages) => {
          ...
          handleDOMEvents: {
            drop(view, event) {
              ...
              Promise.all(
                images.map((image) => uploadImage(image, uploadImagesPath))
              ).then((uploadedImages) => {
                handleUploadedImages(uploadedImages);
              });
            },
