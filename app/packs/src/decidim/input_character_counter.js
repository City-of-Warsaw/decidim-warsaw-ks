...
export default class InputCharacterCounter {
  ...
  constructor(input) {
    if (input.data("disable-character-counter") === true) {
      return;
    }
    ...
    if (this.maxCharacters > 10) {
      if (this.maxCharacters > 100) {
        this.announceThreshold = Math.floor(
          this.maxCharacters * SR_ANNOUNCE_THRESHOLD_RATIO
        );
      } else {
    ...
    if (
      ...
      (this.maxCharacters > 0 || this.minCharacters > 0)
    ) {
  ...
  getScreenReaderLength() {
    ...
    if (this.maxCharacters < 10) {
      ...
      this.maxCharacters - currentLength <=
      this.announceEveryThreshold
    ) {
      return currentLength;
    }
...
const createCharacterCounter = ($input) => {
  if (typeof $input !== "undefined" && $input.length) {
    $input.data(
      ...
      new InputCharacterCounter($input)
    );
  }
};
