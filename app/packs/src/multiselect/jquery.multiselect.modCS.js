...
(function ($) {
  var defaults = {
    ...
    submitForm: true, // show submit form button
    minHeight: 200, // minimum height of option overlay
    maxHeight: 510, // maximum height of option overlay
  ...
  MultiSelect.prototype = {
    ...
    load: function () {
      ...
      // CD-mod.: find additional classes & styles
      var customClass = $(instance.element).data("class");
      var customStyle = $(instance.element).data("style");
      // add option container
      // CD-mod.: add additional classes
      $(instance.element).after(
        '<div id="ms-list-' +
          ...
          '" style="' +
          customStyle +
          '"><button type="button" aria-haspopup="true" aria-expanded="false"><span>None Selected</span></button><div class="ms-options"><ul></ul></div></div>'
      );
      ...
      $(document)
        ...
        .on("keydown", function (event) {
          if ((event.keyCode || event.which) == 27) {
            // esc key
            $(this).find(".ms-options-wrap.ms-active>button").focus();
      ...
      placeholder
        .on("mousedown", function (event) {
          ...
          optionsWrap
            ...
            .attr(
              "aria-expanded",
              optionsWrap.parent().find(">button").attr("aria-expanded") ===
                "true"
                ? "false"
                : "true"
            );
      ...
      optionsList.before('<div class="dropdown-buttons"></div>');
      var buttonsContainer = optionsWrap.find(".dropdown-buttons");

      if (instance.options.submitForm) {
        buttonsContainer.append(
          '<a href="#" class="ms-submitform global" role="button">Wybierz</a>'
        );
        buttonsContainer.append(
          '<a href="#" class="ms-submitform global ms-clearall" role="button">Wyczyść</a>'
        );
      }
      ...
      optionsWrap.on(
        "click",
        ".ms-submitform:not(.ms-clearall)",
        function (event) {
          event.preventDefault();

          var wrapper = $(this).closest(".ms-options-wrap");
          wrapper.removeClass("ms-active");
          wrapper.find("> button").attr("aria-expanded", "false");
        }
      );

      // mod. CS: handle clear all option
      ...
      optionsWrap.on("click", 'input[type="checkbox"]', function () {
        if (instance.options.max === 1) {
          $(instance.element)
            .siblings("#ms-list-" + instance.listNumber + ".ms-options-wrap")
            .find("> .ms-options > ul > li")
            .removeClass("selected");
          ...
          $(instance.element).trigger("click.ms-hideopts");
          return;
        }
      ...
      // BIND Enter/Space KEYDOWN EVENTS
      optionsWrap.on("keydown", 'input[type="checkbox"]', function (event) {
        var code = event.keyCode || event.which;
        if (code == 13 || code == 32) {
          event.preventDefault();
    ...
    loadOptions: function (options, overwrite, updateSelect) {
        ...
        // OPTGROUP
          ...
          if (appendContainer) {
            ...
            if (instance.options.selectGroup) {
              container.append(
                '<a href="#" class="ms-selectall" role="button">' +
      ...
      if (instance.options.selectAll) {
        headerContainer.append(
          '<a href="javascript:void(0)" class="ms-selectall global" role="button">' +
            instance.options.texts.selectAll +
            "</a>"
        );
      }
      ...
      if (!selOpts.length) {
        ...
        var $testSpan = $("<span>", {
          css: {
            ...
            position: "absolute",
            whiteSpace: "nowrap",
            fontFamily: placeholderTxt.css("font-family"),
            fontSize: placeholderTxt.css("font-size"),
            fontWeight: placeholderTxt.css("font-weight"),
            padding: placeholderTxt.css("padding"),
            margin: placeholderTxt.css("margin"),
          },
        }).appendTo("body");

        var finalText = "";
        var totalSelected = selectVals.length;
        var buttonWidth = placeholder.width() - 20;

        for (var i = 0; i < selOpts.length; i++) {
          var testText = i === 0 ? selOpts[i] : finalText + ", " + selOpts[i];

          if ($testSpan.text(testText).width() <= buttonWidth) {
            finalText = testText;
          } else {
            finalText =
              i === 0
                ? instance.options.texts.selectedOptions + " " + totalSelected
                : finalText + "... (" + totalSelected + ")";
            break;
          }
        }
        ...
        var hasCount = finalText.includes("(" + totalSelected + ")");
        var hasEllipsis = finalText.includes("...");
        var isCountOnly =
          finalText ===
          instance.options.texts.selectedOptions + " " + totalSelected;

        if (hasCount || hasEllipsis || isCountOnly) {
          placeholderTxt.html(
            finalText +
