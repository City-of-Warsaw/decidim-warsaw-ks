...
Decidim::Meetings::MapHelper.module_eval do
  ...
  # Added many locations from process selected_scopes, when there is no another locations
  #
  # Changed elements:
  ...
  # - startDate - meeting start date in format: :decidim_short_cut_zero
  ...
  def meetings_data_for_map(meetings)
    meetings.flat_map do |meeting|
      meeting.coordinates_on_map.map do |coords|
        meetings_map_attrs(meeting).merge(coords)
      end
    end
  end

  def meetings_map_attrs(meeting)
    title = translated_attribute(meeting.title)
    {
      ...
      startDate: l(meeting.start_time, format: :decidim_short_cut_zero),
      ...
      past: meeting.past? ? true : false,
      icon: if meeting.online?
      ...
      locationDetails: if meeting.online?
                         link_to(("Link do spotkania<span class='show-for-sr'> #{title}</span>").html_safe, meeting.online_meeting_url, target: "_blank")
                       else
      ...
      meetingType: meeting.type_of_meeting,
      processTitle: translated_attribute(meeting.participatory_space.title),
      area: meeting.participatory_space.area,
      areaId: meeting.participatory_space.decidim_area_id,
