...
Decidim.configure do |config|
  ...
  config.available_locales = %w(pl en)
  ...
  config.maps = {
    provider: :osm,
    osm: ENV.fetch('OSM_URL'),
    nominatim: ENV.fetch('NOMINATIM_URL'),
    api_key: Rails.application.secrets.maps[:api_key],
    dynamic: {
      tile_layer: {
        # url: "https://tiles.example.org/{z}/{x}/{y}.png?key={apiKey}&{foo}",
        url: "#{ENV.fetch('OSM_URL')}/tile/{z}/{x}/{y}.png",
        api_key: false,
        foo: "bar=baz",
        attribution: %(
    ...
    geocoding: { host: ENV.fetch('NOMINATIM_URL'), use_https: true }
  }
